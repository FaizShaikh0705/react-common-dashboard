...
function CentersLeads(props) {
  ...
  const getPostData = () => {
    ...
    firebase.database().ref(`centerDetailsFormData`).get()
      .then((response) => {
  ...
  const handleDelete = (postId, e) => {
    if (window.confirm("Are you sure you want to delete the Post?")) {
      ...
      firebase.database().ref(`centerDetailsFormData/${postId}`).remove()
        .then((response) => {
          ...
          <div className="row justify-content-center">
            <div className="col-lg-11">
              <div className="add-teacher-profile pb-3">
                <div className="title">
                  <h2 id="teach_profile">Centers Leads</h2>
                </div>
              </div>
              <div class="form-row">
